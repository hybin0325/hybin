package com.edas.edasmessage.redis;
...
public class BloomFilter<E> {
	...
	// MD5 gives good enough accuracy in most circumstances. Change to SHA1 if it's
	// needed
	 ...
	 * size of bloom filter and the number of hash functions is estimated to match
	...
	public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
		this((int) Math.ceil((int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))
				* expectedNumberOfElements / Math.log(2)), // m = ceil(kn/ln2)
	...
	public void add(byte[] bytes) {
		...
		for (int hash : hashes) {
			...
			redisTemplate.opsForValue().setBit(RedisConsts.CRAWLER_BLOOMFILTER, Math.abs(hash % sizeOfBloomFilter),
					true);
		}
	}
	...
	public void addAll2(Collection<? extends E> c) {
	...
	public boolean contains(byte[] bytes) {
		int[] hashes = createHashes(bytes, numberOfHashFunctions);
		for (int hash : hashes) {
			if (!redisTemplate.opsForValue().getBit(RedisConsts.CRAWLER_BLOOMFILTER,
					Math.abs(hash % sizeOfBloomFilter))) {
				return false;
			}
		}
		return true;
