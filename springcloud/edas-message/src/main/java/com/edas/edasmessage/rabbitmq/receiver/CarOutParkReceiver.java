...
import com.edas.edasmessage.entity.mq.CarOutParkMsg;
...
@Slf4j
@Component
public class CarOutParkReceiver {

	//@RabbitListener(bindings = @QueueBinding(value = @Queue(value = Constants.CARINPARK_OUT_LIST_QUEUE, durable = "true"), exchange = @Exchange(name = Constants.CAROUTPARK_EXCHANGE, durable = "true", type = "topic"), key = "caroutpark.*"))
	// @RabbitHandler // 如果有消息过来，在消费的时候调用这个方法
	public void onCarOutParkMessage(@Payload CarOutParkMsg msg, @Headers Map<String, Object> headers, Channel channel)
			throws IOException {
		// 消费者操作
		log.info("---------收车出到消息，开始消费---------");
