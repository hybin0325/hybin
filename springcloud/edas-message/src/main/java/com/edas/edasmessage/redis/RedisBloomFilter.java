...
public class RedisBloomFilter {
	...
	public boolean contains(String value) {
		...
		for (Hash f : func) {
			...
			boolean redisIssetRes = redisTemplate.opsForValue().getBit(REDIS_CACHE_KEY + hashValue % 10, hashValue);
			if (!redisIssetRes) {
				return false;
			}
	...
	public void addValue(String value) {
		...
		for (Hash f : func) {
			int hashValue = f.hash(value);
			redisTemplate.opsForValue().setBit(REDIS_CACHE_KEY + hashValue % 10, hashValue, true);
		}
	}
