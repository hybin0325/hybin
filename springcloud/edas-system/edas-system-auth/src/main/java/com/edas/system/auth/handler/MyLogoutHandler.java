...
public class MyLogoutHandler implements LogoutHandler{
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
		...
		/*
		 * Claims claims = JwtUtil.getClaim(token.substring(head.length())); if (claims
		 * == null){ request.setAttribute("userName",null); }else { String userName =
		 * claims.getSubject(); request.setAttribute("userName",userName); }
		 */
    }
}
