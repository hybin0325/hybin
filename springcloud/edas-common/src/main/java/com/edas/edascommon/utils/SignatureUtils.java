	...
	public static void validateParams(Map<String, String> paramsMap) throws Exception {
		...
		if (!SignatureUtils.SignType.contains(paramsMap.get("signType"))) {
			throw new IllegalArgumentException(
					String.format("signType必须为:%s,%s", SignatureUtils.SignType.MD5, SignatureUtils.SignType.SHA256));
		}
		try {
...
//                }
	 ...
	 * 得到签名
	 *
	 * @param paramMap     参数集合不含clientSecret 必须包含clientId=客户端ID signType =
	 *                     SHA256|MD5 签名方式 timestamp=时间戳 nonce=随机字符串
